...
curl -fsSL "$REPO_RAW/appliance/server.py?v=$CACHE_BUST" -o "$APP_ROOT/server.py"
curl -fsSL "$REPO_RAW/appliance/server_entry.py?v=$CACHE_BUST" -o "$APP_ROOT/server_entry.py"
curl -fsSL "$REPO_RAW/appliance/index.html?v=$CACHE_BUST" -o "$APP_ROOT/index.html"
curl -fsSL "$REPO_RAW/appliance/restore_engine.py?v=$CACHE_BUST" -o "$APP_ROOT/restore_engine.py"
curl -fsSL "$REPO_RAW/appliance/installer_engine.py?v=$CACHE_BUST" -o "$APP_ROOT/installer_engine.py"
curl -fsSL "$REPO_RAW/appliance/installer_engine_v2.py?v=$CACHE_BUST" -o "$APP_ROOT/installer_engine_v2.py"
curl -fsSL "$REPO_RAW/appliance/installer_engine_v3.py?v=$CACHE_BUST" -o "$APP_ROOT/installer_engine_v3.py"
curl -fsSL "$REPO_RAW/appliance/sync-license-state.sh?v=$CACHE_BUST" -o "$APP_ROOT/sync-license-state.sh"
